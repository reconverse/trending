...
## Multiple models
Sometimes we want to fit multiple models at once and for this we provide the
`fit_list()` function. This function also provides a parameter `as_tibble`
which allows the user to specify whether they want a `trending_fit_list` or
`trending_fit_tibble` object to be returned.
...
f_list <- fit_list(models, fitting_data)
get_result(f_list)[[2]]
(f_tibble <- fit_list(models, fitting_data, as_tibble = TRUE))
get_result(f_tibble)[[2]]
```

Both of these objects can then be used with `predict()` which has a similar
`as_tibble` parameter.
```{r}
p_list <- predict(f_list, pred_data)
get_result(p_list)[[2]]

(p_tibble <- predict(f_list, pred_data, as_tibble = TRUE))
get_result(p_tibble)[[2]]
