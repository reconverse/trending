...
## Main features
  ...
  - **Error and warning catching:** The provided methods for `fit` and
    ...
    these captured values in a single-row tibble. These results can then
    be handled with additional generics `is_ok` and `ok` to determine
    whether the fitting / prediction functionality has succeeded and
    return successful results respectively.
